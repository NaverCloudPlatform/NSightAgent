...
class Collector(object):
    ...
    def read(self):
        try:
            out = self.process.stderr.read().decode(encoding='ascii')
            if out:
        ...
        try:
            self.buffer += self.process.stdout.read().decode(encoding='ascii')
            if len(self.buffer):
    ...
    def shutdown(self):
        print('------------ shutting down collector %s' % self.name)
        if not self.process:
            return
...
def kill(pid):
    try:
        ...
        LOG.exception('kill process[%i] error: %s', pid, e)
